...
def get_state(cpu, address):
    ...
    if not (block_num in caches[cpu][set_num]):
        ...
        return caches[cpu][set_num].peek(block_num)
...
def set_state_bypass_lru_bypass_lru(cpu, address, state):
    block_num = int(math.floor(address / args.block_size))
    set_num = block_num % sets_count
    ...
    if state == "I":
        ...
        scaches[cpu][set_num].set_bypass_lru(block_num, state)
...
try:
    print("Initialising...")
    for i in range(0, args.processor_count):
        filename = args.input_file + "/" + args.input_file + str(i+1) + '.PRG'
    ...
    print("")
    for i in range(0, args.processor_count):
        print("CPU " + str(i) + " misses: " + str(misses[i]) + "/" + str(accesses[i]))
    print("Traffic: " + str(bus_traffic) + " bytes")
    for i in range(0, args.processor_count):
        print("CPU " + str(i) + " cycles: " + str(cycles[i]))
    # print(stores[0] / float(accesses[0]))
    # print(loads[0] / float(accesses[0]))
